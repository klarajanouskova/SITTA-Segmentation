...
# BEST_METHOD_LR_COCO = {'tent': 5e-4, 'ref': 5e-4, 'pl': 5e-2, 'augco': 5e-3, 'adv': 1e-7, 'iou': 1e-2}
# BEST_METHOD_LOSS_NAME_COCO = {'tent': 'iou', 'ref': 'iou', 'pl': 'iou', 'augco': 'iou', 'adv': 'iou', 'iou': 'iou'}
# BEST_METHOD_N_ITER_COCO = {'tent': 2, 'ref': 5, 'pl': 8, 'augco': 10, 'adv': 0, 'iou': 4}
# BEST_METHOD_NORM_ONLY_COCO = {'tent': False, 'ref': False, 'adv': False, 'iou': True, 'pl': True, 'augco': True,
#                               'adv': False, 'iou': True}


TestConfig = TestConfigCOCO if BASE_DATASET == 'coco' else TestConfigGTA5

class RefTestConfig(TestConfig):
    learn_methods = ['ref']


class TentTestConfig(TestConfig):
    learn_methods = ['tent']


class IouTestConfig(TestConfig):
    learn_methods = ['iou']


class AdvTestConfig(TestConfig):
    learn_methods = ['adv']


class AugcoTestConfig(TestConfig):
    learn_methods = ['augco']


class PlTestConfig(TestConfig):
    learn_methods = ['pl']


class DummyTestConfig(TestConfig):
    learn_methods = ['tent']
    optim = 'sgd'
    # defaults
    n_samples = 4
    method_n_iter = {'tent': 2, 'ref': 2}


class PlAnalysisConfig(TestConfig):
    learn_method = 'pl'
    loss_name = 'iou'
    n_samples = 10
    method_n_iter = {'pl': 10}
...
class RefAnalysisConfig(TestConfig):
    learn_method = 'ref'
    loss_name = 'iou'
    n_samples = 10
    method_n_iter = {'ref': 5}
...
def test_tta(cfg, tta, dataset, samples, res_subfolder=None):
    ...
    ignore_index = 255
    it_iou_metrics_all = [MulticlassJaccardIndex(num_classes=num_classes, ignore_index=ignore_index, average=None) for _ in
                          range(cfg.tta.n_iter + 1)]
    it_total_acc_weighted = [MulticlassAccuracy(num_classes=num_classes, ignore_index=ignore_index, average='micro') for _ in
                            range(cfg.tta.n_iter + 1)]
    it_dice = [MulticlassF1Score(num_classes=num_classes, ignore_index=ignore_index, average='macro') for _ in
                            range(cfg.tta.n_iter + 1)]
...
def get_dataset(cfg):
    ...
    elif cfg.name == "acdc-night":
        dataset = get_acdc(condition='night')
    elif cfg.name == "acdc-fog":
        dataset = get_acdc(condition='fog')
    elif cfg.name == "acdc-rain":
        dataset = get_acdc(condition='rain')
    elif cfg.name == "acdc-snow":
        dataset = get_acdc(condition='snow')
    elif cfg.name == "coco":
        dataset = get_coco(split='val')
    elif cfg.name == "voc":
        dataset = get_voc(split="val")
...
def mask_analysis(cfg):
    ...
    def get_tta(analysis_cfg):
        learn_method = analysis_cfg.learn_method
        lr = analysis_cfg.BEST_METHOD_LR_GTA5[learn_method]
        n_iter = analysis_cfg.BEST_METHOD_N_ITER_GTA5[learn_method]
        cfg.tta.loss_name = analysis_cfg.loss_name
        cfg.tta.lr = lr
        cfg.tta.n_iter = n_iter
        cfg.tta.optim = analysis_cfg.optim
        cfg.tta.train_norm_only = analysis_cfg.train_norm_only[learn_method]
        tta = TestTimeAdaptor(cfg=cfg, tta_method=learn_method, base_dataset=BASE_DATASET)
        return tta
    ...
    # ref_tta
    an_cfg = RefAnalysisConfig
    tta = get_tta(an_cfg)
    test_tta_analysis(cfg=cfg, tta=tta, dataset=dataset, samples=an_cfg.n_samples)
...
def main_test(cfg):
    ...
    method2cfg = {'tent': TentTestConfig, 'pl': PlTestConfig, 'ref': RefTestConfig,
                  'augco': AugcoTestConfig, 'iou': IouTestConfig, 'adv': AdvTestConfig
                  }
    ...
    test_cfg = method2cfg[method]
    # test_cfg = DummyTestConfig

    if 0 in data_idxs:
        cfg.base_data.name = "voc" if BASE_DATASET == "coco" else "cityscapes"
        print(f"Testing {cfg.base_data.name}")
        dataset = get_dataset(cfg)
        test_sweep(cfg, test_cfg, dataset, res_subfolder='test/')

    if not BASE_DATASET == "coco":
        for c_idx, cond in enumerate(ACDC.VALID_CONDS):
            ...
            # test_sweep(cfg, TestConfig, dataset, res_subfolder=f'test/')


@hydra.main(config_path=f"conf/{'coco' if BASE_DATASET == 'coco' else 'gta'}", config_name="tta_eval",
            version_base=None)
def main_test_all_methods(cfg):
    """
    Benchmark test datasets
    :param cfg:
    :return:
