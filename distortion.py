...
Except fro adversarial perturbation, the methods are taken from
 https://github.com/bethgelab/imagecorruptions/tree/master/imagecorruptions [TODO cite].

These should be used for hparam tuning.

motion blur and snow removed because of wand dependency, may be added later
Also albumentations https://albumentations.ai/docs/ have interesting transforms like fourier domain shift.
These should be sued for testing.

TODO: tru to use these
...
def frost(x, severity=1):
    ...
    # TODO solve path issue
    filename = ['../frost/frost1.png',
                '../frost/frost2.png',
                '../frost/frost3.png',
                '../frost/frost4.png',
                '../frost/frost5.png',
                '../frost/frost6.png'][idx]
...
# distortions that don't change the segmentation mask too much
distortions_cov = OrderedDict([
    ('gaussian_noise', gaussian_noise),
    ('shot_noise', shot_noise),
    ('impulse_noise', impulse_noise),
    ('defocus_blur', defocus_blur),
...
def test_distortions():
    ...
    for dist_name, dist_fun in all_distortions.items():
        levels = [3, 5]
        os = [dist_fun(F_tfm.to_pil_image(inp[0]), severity) for severity in levels]
        plt.subplot(1, len(os) + 1, 1)
        # show original
        plt.imshow(np.array(F_tfm.to_pil_image(inp[0])).astype(np.uint8))
        plt.title('original')
        plt.axis('off')
        for idx, (level, o) in enumerate(zip(levels, os)):
            plt.subplot(1, len(os) + 1, idx + 2)
            plt.imshow(np.array(o).astype(np.uint8))
            plt.title(f'{dist_name} {level + 1}')
            plt.axis('off')
        plt.tight_layout()
        plt.show()
...
def get_random_corruption_fun(severities=None):
    ...
    distortion = np.random.choice(list(distortions_cov.values()))
    # select a random severity
    if severities is None:
        severity = np.random.randint(0, 6)
    else:
